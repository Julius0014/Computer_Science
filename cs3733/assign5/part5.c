...
void printlist(struct PCB* iHead){
    while (iHead != NULL){
        printf("%i->",iHead->PR);
        iHead = iHead->next;
    }
...
void updateMAXMIN() {
    ...
    printf("NEW MIN : PR = %i  MIN = %i\n",min->PR,min->CPUBurst[min->cpuindex]);
    min->next = NULL;
    min->prev = NULL;
}
/*LinkList insert a node*/
...
void updateList(struct PCB *Mmin) {
    ...
    if(Mmin->prev == NULL) 
     ...
     if( Mmin->next == NULL)
        printf("NULL Next\n");
    if(Mmin->prev == NULL && Mmin->next == NULL){
    printf("After\n");
    printlist(Head);
        return;
    }
...
int SJF_Scheduling() {
  printf("start1 minpr = %i\n",min->PR);
  updateList(min);
   updateMAXMIN();
   int x = handlenode(min);
   updateMAXMIN();
  printf("After PR = %i  MIN = %i\n",min->PR,min->CPUBurst[min->cpuindex]);
  return x;
...
int handlenode(struct PCB *temp){
    int sleep;
    Total_job++;
    printf("PR = %i  MIN = %i\n",temp->PR,temp->CPUBurst[temp->cpuindex]);
    temp->next = NULL;
    temp->prev = NULL;
